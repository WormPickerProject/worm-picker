...
GenerateWorkstationPlacePlateTask::GenerateWorkstationPlacePlateTask(
...
const std::unordered_map<std::string, TaskData>& GenerateWorkstationPlacePlateTask::getTaskDataMap() const 
{
    return task_data_map_;
}

std::pair<char, int> GenerateWorkstationPlacePlateTask::parseWorkstationName(const std::string& name) const 
{
...
std::string GenerateWorkstationPlacePlateTask::generateTaskName(char row_letter, int col_number) const
{
    constexpr char prefix[] = "placePlateWorkStation:";
    return prefix + std::string(1, row_letter) + std::to_string(col_number);
}

std::vector<std::shared_ptr<StageData>> GenerateWorkstationPlacePlateTask::createStagesForTask(
...
Coordinate GenerateWorkstationPlacePlateTask::calculateDerivedPoint(const Coordinate& coord, char row_letter) const
{
    constexpr double XY_OFFSET = 0.008;
...
std::shared_ptr<StageData> GenerateWorkstationPlacePlateTask::createMoveToPointStage(const Coordinate& coord) const
{
